...
import os 
root_dir = os.path.join('/Users/yangboz/git/Fruit-Images-Dataset/')
...
with open(root_dir + '/src/image_classification/utils/labels') as f:
    labels = f.readlines()
...
def process_TF(image_path):
    ...
    #image_path = "/Users/yangboz/git/Fruit-Images-Dataset/test-multiple_fruits/apple.jpg"
        ...
        FLAGS = tf.app.flags.FLAGS
        #tf.app.flags.DEFINE_string('image_path','images\\Lemon2.jpg',image_path)
    ...
    # restore the trained model from the saved checkpoint; provide the path to the meta file
        saver = tf.train.import_meta_graph(fruit_models_dir + 'model.ckpt.meta')
    # provide the path to the folder containing the checkpoints
        saver.restore(sess, tf.train.latest_checkpoint(fruit_models_dir))
    ...
    # obtain the output layer by name and apply softmax on in in order to obtain an output of probabilities
        ...
        return process_image(sess, X, softmax, image, height, width, depth)
